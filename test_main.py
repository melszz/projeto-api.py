...
def test_nao_permite_sala_inexistente():
    resposta = client.post("/reservas", json=payload_reserva(sala_id=999))
...
def test_nao_permite_conflito_de_horario():
    client.post("/reservas", json=payload_reserva())
    resposta = client.post(
        "/reservas",
...
def test_buscar_reserva_por_id():
    criada = client.post("/reservas", json=payload_reserva()).json()
...
def test_excluir_reserva():
    criada = client.post("/reservas", json=payload_reserva()).json()
    resposta = client.delete(f"/reservas/{criada['id']}")
    assert resposta.status_code == 204
    assert client.get(f"/reservas/{criada['id']}").status_code == 404


def test_filtro_por_sala():
    client.post("/reservas", json=payload_reserva(sala_id=1))
    client.post(
        "/reservas",
